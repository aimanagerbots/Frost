      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-text-default">{order.assignedRep}</p>
          </div>
          <div>
          ...
          <div>
            <h4 className="text-xs font-semibold uppercase text-text-muted tracking-wider">Total</h4>
            <p className="mt-1 text-sm font-semibold text-text-bright">${order.total.toLocaleString()}</p>
          </div>
        </div>

        {/* Line Items */}
        <div>
          <h4 className="mb-2 text-xs font-semibold uppercase text-text-muted tracking-wider">Line Items ({order.items.length})</h4>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-default bg-elevated">
                  ...
                  <th className="px-3 py-2 text-right text-text-muted font-medium">Unit</th>
                  <th className="px-3 py-2 text-right text-text-muted font-medium">Total</th>
                </tr>
              </thead>
              <tbody>
                {order.items.map((li) => (
                  <tr key={li.id} className="border-b border-default last:border-0">
                    <td className="px-3 py-2">
                      <span className="text-text-default">{li.productName}</span>
                      {li.batchNumber && (
                        <span className="ml-1 text-[10px] text-text-muted">({li.batchNumber})</span>
                      )}
                    </td>
                    <td className="px-3 py-2 text-right text-text-default">{li.quantity}</td>
                    <td className="px-3 py-2 text-right text-text-muted">${li.unitPrice}</td>
                    <td className="px-3 py-2 text-right text-text-bright font-medium">${li.lineTotal.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-elevated">
                  <td colSpan={3} className="px-3 py-2 text-right text-text-muted font-medium">Subtotal</td>
                  <td className="px-3 py-2 text-right text-text-default">${order.subtotal.toLocaleString()}</td>
                </tr>
                <tr className="bg-elevated">
                  <td colSpan={3} className="px-3 py-2 text-right text-text-muted font-medium">Tax</td>
                  <td className="px-3 py-2 text-right text-text-default">${order.tax.toLocaleString()}</td>
                </tr>
                <tr className="bg-elevated border-t border-default">
                  <td colSpan={3} className="px-3 py-2 text-right text-text-bright font-semibold">Total</td>
                  <td className="px-3 py-2 text-right text-text-bright font-semibold">${order.total.toLocaleString()}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        {order.notes && (
          <div>
            ...
            <p className="mt-1 text-sm text-text-default">{order.notes}</p>
          </div>
        )}

        {/* Timeline */}
        <div>
          <h4 className="mb-2 text-xs font-semibold uppercase text-text-muted tracking-wider">Timeline</h4>
