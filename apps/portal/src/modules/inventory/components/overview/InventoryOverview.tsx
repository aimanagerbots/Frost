    ...
    <div className="space-y-5">
      ...
      {metrics && (
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <MetricCard label="Total Inventory Value" value={`$${(metrics.totalValue / 1000).toFixed(0)}K`} accentColor="#059669" trend={{ value: 12, direction: 'up' }} />
          <MetricCard label="Below Reorder Point" value={metrics.belowReorderPoint} accentColor={metrics.belowReorderPoint > 5 ? '#FB7185' : '#FBBF24'} />
          <MetricCard label="COA Tests Pending" value={metrics.coaPending} accentColor="#FBBF24" />
          <MetricCard label="Avg Days in Pipeline" value={`${metrics.avgDaysInPipeline}d`} accentColor="#3B82F6" trend={{ value: 5, direction: 'down' }} />
          <MetricCard label="Expiring COAs (30d)" value={metrics.expiringCOAs} accentColor={metrics.expiringCOAs > 0 ? '#FBBF24' : '#22C55E'} />
        </div>
      )}
