...
import { NonCannabisTab } from './NonCannabisTab';
import { AlertsTab } from './AlertsTab';
import { COAManagerPage } from '@/modules/coa/components/COAManagerPage';
...
export function InventoryPage() {
  ...
  const [activeTab, setActiveTab] = useState<InventoryTab>(tabParam && TABS.some((t) => t.key === tabParam) ? tabParam : 'overview');
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* Tab Content */}
      {activeTab === 'coa' && <COAManagerPage />}
      {activeTab === 'non-cannabis' && <NonCannabisTab />}
      {activeTab === 'alerts' && <AlertsTab />}

      {(activeTab === 'overview' || activeTab === 'cannabis') && (
        <>
