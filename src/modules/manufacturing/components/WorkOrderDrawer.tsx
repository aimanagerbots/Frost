  ...
  return (
    <DrawerPanel open={open} onClose={() => { onClose(); setLocalStatus(null); }} title={workOrder.title} width="lg">
      <div className="space-y-6">
        ...
        {/* Description */}
        <p className="text-sm text-default">{workOrder.description}</p>

        {/* Readiness Progress */}
        <div className="rounded-lg border border-default bg-base p-3">
          <div className="text-xs text-muted mb-2">Readiness State Transition</div>
        ...
        {/* Materials Table */}
        <div>
          <h3 className="text-xs font-medium text-muted mb-2">Input Materials</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-default bg-base">
                  ...
                  <th className="px-3 py-2 text-left text-xs font-medium text-muted">Type</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-muted">Qty</th>
                  <th className="px-3 py-2 text-center text-xs font-medium text-muted">Available</th>
                </tr>
              </thead>
              <tbody>
                {workOrder.inputMaterials.map((m, i) => (
                  <tr key={i} className="border-b border-default/50">
                    ...
                    <td className="px-3 py-2 text-right text-default">{m.quantity} {m.unit}</td>
        ...
        <div className="rounded-lg border border-default bg-base p-3">
          ...
          <div className="text-sm text-bright">{workOrder.outputProduct}</div>
          <div className="text-xs text-muted">{workOrder.outputQuantity} {workOrder.outputUnit}</div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="text-sm text-bright">{workOrder.assignee}</div>
            <div className="text-xs text-muted">{workOrder.assigneeRole}</div>
          </div>
          <div>
            <div className="text-xs text-muted mb-1">Time</div>
            <div className="text-sm text-bright">Est: {workOrder.estimatedMinutes} min</div>
            {workOrder.actualMinutes && (
              <div className="text-xs text-muted">Actual: {workOrder.actualMinutes} min</div>
            )}
          </div>
        </div>
        ...
        {workOrder.linkedOrderIds.length > 0 && (
          <div>
            <div className="text-xs text-muted mb-1">Linked Orders</div>
            <div className="flex flex-wrap gap-1">
              {workOrder.linkedOrderIds.map((id) => (
                <span key={id} className="rounded-md bg-elevated px-2 py-0.5 text-xs text-default">{id}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {workOrder.notes && (
          <div className="rounded-lg border border-default bg-base p-3">
            ...
            <div className="text-sm text-default">{workOrder.notes}</div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              ...
              <span className="text-default">{new Date(workOrder.createdAt).toLocaleString()}</span>
            </div>
            {workOrder.startedAt && (
              <div className="flex justify-between">
                <span className="text-muted">Started</span>
                <span className="text-default">{new Date(workOrder.startedAt).toLocaleString()}</span>
              </div>
            )}
            {workOrder.completedAt && (
              <div className="flex justify-between">
                <span className="text-muted">Completed</span>
                <span className="text-default">{new Date(workOrder.completedAt).toLocaleString()}</span>
              </div>
            )}
          </div>
        </div>
