    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-default bg-card p-4">
          <h3 className="mb-3 text-sm font-semibold text-bright">Business Info</h3>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between">
              <dt className="text-muted">Legal Name</dt>
              <dd className="text-default">{account.name}</dd>
            </div>
            {account.dba && account.dba !== account.name && (
              <div className="flex justify-between">
                <dt className="text-muted">DBA</dt>
                <dd className="text-default">{account.dba}</dd>
              </div>
            )}
            <div className="flex justify-between">
              <dt className="text-muted">License</dt>
              <dd className="text-default">{account.licenseNumber}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted">License Exp.</dt>
            ...
            <div className="flex justify-between">
              ...
              <dd className="text-right text-default">
            ...
            <div className="flex justify-between">
              ...
              <dd className="text-default">{account.region}</dd>
            </div>
            <div className="flex justify-between">
        ...
        <div className="rounded-xl border border-default bg-card p-4">
          <h3 className="mb-3 text-sm font-semibold text-bright">Delivery Preferences</h3>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between">
              <dt className="text-muted">Window</dt>
              <dd className="text-default">{account.deliveryPreferences.window}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted">Contact</dt>
              <dd className="text-default">{account.deliveryPreferences.contactName}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-muted">Phone</dt>
              <dd className="text-default">{account.deliveryPreferences.contactPhone}</dd>
            </div>
            {account.deliveryPreferences.instructions && (
              <div>
                <dt className="text-muted">Instructions</dt>
                <dd className="mt-1 text-default">{account.deliveryPreferences.instructions}</dd>
              </div>
            )}
          </dl>

          <h3 className="mb-3 mt-6 text-sm font-semibold text-bright">Payment</h3>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between">
              <dt className="text-muted">Preferred Method</dt>
              <dd className="text-default uppercase">{account.preferredPaymentMethod}</dd>
            </div>
            <div className="flex justify-between">
      ...
      {/* Category Mix */}
      <div className="rounded-xl border border-default bg-card p-4">
        <h3 className="mb-3 text-sm font-semibold text-bright">Category Mix</h3>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
          {account.categoryMix.map((cm) => (
            <div key={cm.category} className="rounded-lg bg-elevated p-3 text-center">
              <div className="text-lg font-bold text-bright">{cm.percentage}%</div>
              <div className="text-xs capitalize text-muted">{cm.category}</div>
      ...
      {/* Contacts */}
      <div className="rounded-xl border border-default bg-card p-4">
        <h3 className="mb-3 text-sm font-semibold text-bright">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {account.contacts.map((contact) => {
            ...
            return (
              ...
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-card text-sm font-medium text-muted">
                  {initials}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-bright">{contact.name}</span>
