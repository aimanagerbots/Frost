    ...
    <DrawerPanel open={!!playbook} onClose={onClose} title={playbook.name} width="lg">
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-muted">{playbook.steps.length} steps</span>
          <span className="text-xs text-muted">·</span>
          <span className="text-xs text-muted">{playbook.estimatedDuration}</span>
          <span className="text-xs text-muted">·</span>
          <span className="text-xs text-success">{playbook.successRate}% success rate</span>
        </div>

        <p className="text-sm text-muted leading-relaxed">{playbook.description}</p>
              ...
              return (
                <div key={step.id} className="relative flex gap-4">
                  ...
                  <div className="relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-default bg-card">
                    <span className="text-xs font-bold text-bright">{step.order}</span>
                  </div>
                  ...
                  <div className="flex-1 rounded-xl border border-default bg-base p-3">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-bright">{step.title}</span>
                    ...
                    {isExpanded && (
                      <div className="mt-3 space-y-3 border-t border-default pt-3">
                        <p className="text-sm text-muted leading-relaxed">{step.instructions}</p>
                        {step.aiPrompt && (
                          <div className="flex items-start gap-2 rounded-lg bg-info/10 p-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted">{step.aiPrompt}</p>
                            </div>
                          </div>
                        )}
                      </div>
