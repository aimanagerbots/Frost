            ...
            {(['month', 'week', 'day'] as CalendarViewMode[]).map((v) => (
              <button
                ...
                className={cn(
                  ...
                  view === v ? 'bg-card text-bright' : 'text-muted hover:text-default'
                )}
              >
                {VIEW_LABELS[v]}
              </button>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-default text-muted transition-colors hover:bg-elevated hover:text-default"
            aria-label="Previous"
          >
          ...
          <button
            ...
            className="rounded-lg border border-default px-3 py-1 text-xs font-medium text-muted transition-colors hover:bg-elevated hover:text-default"
          >
            Today
          </button>
          <button
          ...
          <h2 className="ml-2 text-sm font-semibold text-bright">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              {selectedEvent.allDay && (
                <div className="flex items-center gap-2 text-sm">
                  <Clock className="h-4 w-4 text-muted" />
                  <span className="text-default">All day</span>
                </div>
              )}
            </div>
            ...
            {selectedEvent.description && (
              <div>
                <h4 className="mb-1 text-xs font-medium uppercase tracking-wide text-muted">Description</h4>
                <p className="text-sm text-default leading-relaxed">{selectedEvent.description}</p>
              </div>
            )}
            ...
            {selectedEvent.attendees && selectedEvent.attendees.length > 0 && (
              <div>
                <h4 className="mb-2 text-xs font-medium uppercase tracking-wide text-muted">Attendees</h4>
                <div className="space-y-1.5">
                  {selectedEvent.attendees.map((name) => (
                    <div key={name} className="flex items-center gap-2 text-sm text-default">
            ...
            {selectedEvent.moduleRoute && (
              <div>
                <h4 className="mb-2 text-xs font-medium uppercase tracking-wide text-muted">Module</h4>
                <div className="flex items-center gap-2 text-sm">
